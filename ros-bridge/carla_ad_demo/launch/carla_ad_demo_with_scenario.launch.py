...
ros_topic_msg_string = "{{ 'scenarios': [{{ 'name': 'FollowLeadingVehicle', 'scenario_file': '{}'}}] }}".format(
    os.path.join(get_package_share_directory('opina_user_data'), '/resource/scenarios/FollowLeadingVehicle_6.xosc'))
...
def generate_launch_description():
    ld = launch.LaunchDescription([
        ...
        launch.actions.IncludeLaunchDescription(
            ...
            launch_arguments={
                'object_definition_file': get_package_share_directory('opina_user_data') + '/resource/vehicles/tesla.json',
                'role_name': launch.substitutions.LaunchConfiguration('role_name')
            }.items()
        ),
        launch.actions.IncludeLaunchDescription(
